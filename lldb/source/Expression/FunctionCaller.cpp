...
FunctionCaller::FunctionCaller(ExecutionContextScope &exe_scope,
                               ...
                               const char *name)
    : llvm::RTTIExtends<FunctionCaller, Expression>(exe_scope),
      m_execution_unit_sp(), m_parser(), m_jit_module_wp(),
      m_name(name ? name : "<unknown>"), m_function_ptr(nullptr),
      m_function_addr(functionAddress), m_function_return_type(return_type),
